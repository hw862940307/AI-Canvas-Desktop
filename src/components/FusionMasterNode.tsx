...
const FusionMasterNode = ({ id, data, selected }: { id: string; data: any; selected?: boolean }) => {
  ...
  const [framework, setFramework] = useState(data?.framework || 'V5.9.1 Product Fusion & Prompt Engineering Master framework.');
  ...
  }, [mainSlots, mode, userPrompt, framework, commands, supportAssets]);
  ...
  const handleFrameworkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setFramework(event.target?.result as string);
      };
...
OUTPUT FORMAT: Provide a comprehensive engineered prompt in plain text.`;
      ...
      if (mode !== 'Mode C' && mainSlots.productMaster) {
        contents.push({ inlineData: { mimeType: "image/png", data: (mainSlots.productMaster as string).split(',')[1] } });
      }
  ...
  const NodeHeader = ({ inFullscreen = false }) => (
    <div className={`flex items-center justify-between px-6 py-5 border-b border-white/5 bg-black/40 backdrop-blur-xl react-flow__node-draghandle ${inFullscreen ? 'py-4 px-8' : ''}`}>
      <div className="flex items-center gap-4">
        <div className={`p-3 bg-blue-600 rounded-2xl shadow-lg ring-4 ring-blue-600/10 ${inFullscreen ? 'scale-110' : ''}`}>
      ...
      <div className="flex items-center gap-2">
        <button 
          ...
          className={`p-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 transition-all border border-white/5 ${inFullscreen ? 'scale-110' : ''}`}
          title={isFullscreen ? "Exit Fullscreen" : "Fullscreen Mode"}
        >
          {isFullscreen ? <Minimize size={inFullscreen ? 20 : 18} /> : <Maximize size={inFullscreen ? 20 : 18} />}
        </button>
        {!inFullscreen && (
          <button onClick={(e) => { e.stopPropagation(); setShowTools(!showTools); }} className={`p-2.5 rounded-xl transition-all border ${showTools ? 'bg-blue-600 border-blue-400' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
    ...
    <div className={`space-y-8 ${inFullscreen ? 'p-0' : ''}`}>
      ...
      <div className={`grid gap-4 ${mode === 'Mode C' ? 'grid-cols-1' : 'grid-cols-2'}`}>
        {mode !== 'Mode C' && (
          <div 
            ...
            className={`aspect-square rounded-[24px] border-2 border-dashed flex flex-col items-center justify-center relative overflow-hidden transition-all group pointer-events-auto ${
              isDragOver.productMaster ? 'border-blue-500 bg-blue-500/20 scale-[1.02]' : 
              mainSlots.productMaster ? 'border-blue-500/30 bg-blue-500/5' : 'border-white/5 bg-white/2 hover:border-blue-500/20'
            }`}
          >
        ...
        <div 
          ...
          className={`aspect-square rounded-[24px] border-2 border-dashed flex flex-col items-center justify-center relative overflow-hidden transition-all group pointer-events-auto ${
            isDragOver.sceneReference ? 'border-blue-500 bg-blue-500/20 scale-[1.02]' :
            mainSlots.sceneReference ? 'border-blue-500/30 bg-blue-500/5' : 'border-white/5 bg-white/2 hover:border-blue-500/20'
        ...
        <AnimatePresence>
          {isFullscreen && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[10000] bg-[#050505] flex flex-col font-sans overflow-hidden"
            >
              {/* Fullscreen Header */}
              <div className="h-20 shrink-0 bg-[#0A0A0A] border-b border-white/5 flex items-center justify-between px-10">
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                >
                    ...
                    <div className={`grid gap-12 ${mode === 'Mode C' ? 'grid-cols-1 max-w-2xl mx-auto' : 'grid-cols-2'}`}>
                        ...
                        <div className="space-y-6 group">
                           ...
                           <div 
                             ...
                             className={`aspect-square bg-black rounded-[48px] border-2 border-dashed shadow-[0_0_100px_rgba(37,99,235,0.05)] relative overflow-hidden transition-all flex items-center justify-center pointer-events-auto ${
                               isDragOver.productMaster ? 'border-blue-500 bg-blue-500/20 scale-[1.02]' : 'border-white/5'
                             }`}
                           >
                      ...
                      <div className="space-y-6 group">
                         ...
                         <div 
                           ...
                           onPaste={(e) => handlePaste(e, 'sceneReference')}
                           className={`aspect-square bg-black rounded-[48px] border-2 border-dashed shadow-[0_0_100px_rgba(37,99,235,0.05)] relative overflow-hidden transition-all flex items-center justify-center pointer-events-auto ${
                             isDragOver.sceneReference ? 'border-blue-500 bg-blue-500/20 scale-[1.02]' : 'border-white/5'
                           }`}
                         >
                  ...
                  {/* Sidebar Panel (Right) */}
                  <div className="w-[440px] shrink-0 bg-[#0A0A0A] border-l border-white/5 flex flex-col shadow-2xl relative z-10 overflow-hidden">
                ...
                <AnimatePresence>
                  {showTools && (
                    <motion.div 
                      ...
                      className="absolute top-20 right-0 bottom-0 w-[440px] bg-[#080808] z-[50] border-l border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
                    >
