...
export function DoubleBoxTransformNode({ id, data, selected }: NodeProps) {
  ...
  const [annotationNotes, setAnnotationNotes] = useState<string>(nodeData.annotationNotes || '');
  
  // Styling States
  const [boxMode, setBoxMode] = useState<'outline' | 'fill'>(nodeData.boxMode || 'outline');
  const [boxOpacity, setBoxOpacity] = useState<number>(nodeData.boxOpacity || 40);
  
  const [activeStylingTarget, setActiveStylingTarget] = useState<'blue' | 'red'>('blue');
  
  const [blueLineWidth, setBlueLineWidth] = useState<number>(nodeData.blueLineWidth || nodeData.lineWidth || 2);
  const [blueLineType, setBlueLineType] = useState<'solid' | 'dashed'>(nodeData.blueLineType || nodeData.lineType || 'dashed');
  const [blueDashGap, setBlueDashGap] = useState<number>(nodeData.blueDashGap || nodeData.dashGap || 5);
  const [blueDashLength, setBlueDashLength] = useState<number>(nodeData.blueDashLength || nodeData.dashLength || 5);

  const [redLineWidth, setRedLineWidth] = useState<number>(nodeData.redLineWidth || nodeData.lineWidth || 2);
  const [redLineType, setRedLineType] = useState<'solid' | 'dashed'>(nodeData.redLineType || nodeData.lineType || 'dashed');
  const [redDashGap, setRedDashGap] = useState<number>(nodeData.redDashGap || nodeData.dashGap || 5);
  const [redDashLength, setRedDashLength] = useState<number>(nodeData.redDashLength || nodeData.dashLength || 5);

  const [arrowColor, setArrowColor] = useState<string>(nodeData.arrowColor || '#fbbf24');
  const [arrowWidth, setArrowWidth] = useState<number>(nodeData.arrowWidth || 1.5);
  const [arrowOpacity, setArrowOpacity] = useState<number>(nodeData.arrowOpacity || 100);
  const [arrowLineType, setArrowLineType] = useState<'solid' | 'dashed'>(nodeData.arrowLineType || 'solid');
  const [arrowDashGap, setArrowDashGap] = useState<number>(nodeData.arrowDashGap || 5);
  const [arrowDashLength, setArrowDashLength] = useState<number>(nodeData.arrowDashLength || 5);
  const [arrowDisplayMode, setArrowDisplayMode] = useState<'LINE' | 'OUTLINE'>(nodeData.arrowDisplayMode || 'LINE');
  const [showSettings, setShowSettings] = useState(false);
  
  // Interaction Logic
  const [isEditing, setIsEditing] = useState(false);
  const [drawMode, setDrawMode] = useState<'blue' | 'red' | null>(null);
  const [boxesVisible, setBoxesVisible] = useState<boolean>(nodeData.blueBox ? true : false);
  const [blueBox, setBlueBox] = useState<BoxState>(nodeData.blueBox || { x: 0, y: 0, width: 0, height: 0 });
  const [redBox, setRedBox] = useState<BoxState>(nodeData.redBox || { x: 0, y: 0, width: 0, height: 0 });
  ...
  useEffect(() => {
    ...
    // Prioritize specific source nodes if needed, but look for any valid URL
    const firstImg = incoming.find(d => d.url || d.imageUrl);
    if (firstImg) {
      const newUrl = firstImg.url || firstImg.imageUrl;
      if (newUrl !== imageUrl) {
        setImageUrl(newUrl);
        updateNodeData(id, { url: newUrl });
      }
    }
  }, [id, getIncomingData, imageUrl, updateNodeData]);
  ...
  const getPrompt = useCallback(() => {
    ...
    const sizeRelation = redBox.width * redBox.height > blueBox.width * blueBox.height ? 'enlarge' : 
                        redBox.width * redBox.height < blueBox.width * blueBox.height ? 'shrink' : 'same_size';
...
最终目标：蓝框内识别出的主体按红框精准变换，源位置背景完美修补，目标位置自然融合，所有指示性 UI 元素彻底消失，图像干净如直拍。`;
  ...
  const prompt = React.useMemo(() => {
    ...
    return getPrompt();
  }, [boxesVisible, blueBox.x, blueBox.y, blueBox.width, blueBox.height, redBox.x, redBox.y, redBox.width, redBox.height, isDragging, isResizing, isDrawing]);
  ...
  const onMouseDown = (e: React.MouseEvent, type?: 'blue' | 'red', action?: 'drag' | 'resize', handle?: string) => {
    e.stopPropagation();
    
    const targetRef = isFullscreen ? fullscreenInnerContainerRef : innerContainerRef;
  ...
  const onMouseMove = useCallback((e: MouseEvent) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      if (isDragging) {
        nextBox.x = Math.max(0, Math.min(100 - initialBox.width, initialBox.x + dx));
        nextBox.y = Math.max(0, Math.min(100 - initialBox.height, initialBox.y + dy));
      } else if (isResizing && resizeHandle) {
        if (resizeHandle.includes('e')) {
          nextBox.width = Math.max(0.1, Math.min(100 - initialBox.x, initialBox.width + dx));
        }
        if (resizeHandle.includes('s')) {
          nextBox.height = Math.max(0.1, Math.min(100 - initialBox.y, initialBox.height + dy));
        }
  ...
  }, [isDragging, isResizing, isDrawing, dragStart, initialBox, activeBox, resizeHandle, drawMode, isFullscreen]);
  ...
  }, [isDrawing, isDragging, isResizing, id, blueBox, redBox, updateNodeData, drawMode, boxMode, blueLineWidth, blueLineType, blueDashGap, blueDashLength, redLineWidth, redLineType, redDashGap, redDashLength, boxOpacity, arrowColor, arrowWidth, arrowOpacity, arrowLineType, arrowDashGap, arrowDashLength, arrowDisplayMode]);
  ...
  }, [id, boxMode, blueLineWidth, blueLineType, blueDashGap, blueDashLength, redLineWidth, redLineType, redDashGap, redDashLength, boxOpacity, arrowColor, arrowWidth, arrowOpacity, arrowLineType, arrowDashGap, arrowDashLength, arrowDisplayMode, updateNodeData]);
  ...
  const renderBox = (type: 'blue' | 'red', box: BoxState) => {
    ...
    const color = isBlue ? `rgba(59, 130, 246, ${boxMode === 'fill' ? boxOpacity / 100 : 0.1})` : `rgba(239, 68, 68, ${boxMode === 'fill' ? boxOpacity / 100 : 0.1})`;
    ...
    const borderColor = isBlue ? `rgba(59, 130, 246, ${boxOpacity/100})` : `rgba(239, 68, 68, ${boxOpacity/100})`;
    const label = isBlue ? '源物体蓝框' : '目标画布红框';
    
    const dashArray = currentLineType === 'dashed' ? `${currentDashLength} ${currentDashGap}` : 'none';
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {isBlue ? <Box size={10 * zoomScale} /> : <Crosshair size={10 * zoomScale} />}
            {label}
          </div>
          <p className={`text-[10px] font-mono mt-1 transition-opacity ${activeStylingTarget === type ? 'text-white font-bold' : 'text-white/60'}`} style={{ fontSize: 8 * zoomScale }}>
            {type === 'blue' ? 'SOURCE_OBJECT' : 'TARGET_ANCHOR'}
          </p>
        </div>

        {/* Resize Handles */}
        {isEditing && ['nw', 'ne', 'sw', 'se', 'n', 's', 'e', 'w'].map((h) => (
          <div
            key={h}
  ...
  const renderScalingArrow = () => {
    ...
    blueCorners.forEach((bc, i) => {
      ...
      const dist = Math.sqrt(Math.pow(rc.x - bc.x, 2) + Math.pow(rc.y - bc.y, 2));
      if (dist > maxDist) {
        maxDist = dist;
    ...
    const arrowDashArray = arrowLineType === 'dashed' ? `${arrowDashLength} ${arrowDashGap}` : 'none';
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-[15] overflow-visible snapshot-hide">
        <defs>
          ...
          <marker
            ...
            orient="auto"
          >
            <path d="M 0 0 L 8 3 L 0 6 Z" fill={arrowColor} fillOpacity={arrowOpacity / 100} />
          </marker>
        </defs>
      ...
      >
        ...
        >
          <div className="flex items-center" style={{ gap: 16 * zoomScale }}>
            ...
            <div>
              <h3 className="font-black text-white tracking-widest uppercase italic" style={{ fontSize: 14 * zoomScale }}>
                Transform_Master
              </h3>
              <div className="flex items-center font-mono text-gray-600 tracking-widest" style={{ gap: 8 * zoomScale, fontSize: 10 * zoomScale, marginTop: 2 * zoomScale }}>
                <span>DB_OBJ_V1.0</span>
                <div className="w-1 h-1 rounded-full bg-indigo-500" />
                <span className="text-indigo-500/60 uppercase">DoubleBox Prompt Engine</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center" style={{ gap: 12 * zoomScale }}>
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase hidden md:inline">Annotate</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase hidden md:inline">Download</span>
            </button>
            <button 
        ...
        <div className="flex-1 flex flex-col p-6 overflow-hidden nodrag" style={{ gap: 20 * zoomScale }}>
          {/* Main Workspace */}
          <div 
            ref={containerRef}
            className={`flex-1 relative bg-black rounded-[24px] overflow-hidden border border-white/5 shadow-inner ${drawMode ? 'cursor-crosshair' : ''}`}
            ...
            >
              {imageUrl ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-800 gap-4 opacity-40">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.4em]">Awaiting Image Input</span>
                </div>
              )}
            ...
            {drawMode && (
              <div className="absolute inset-0 bg-blue-500/5 pointer-events-none flex items-center justify-center">
                <div className="px-4 py-2 bg-black/80 rounded-full border border-white/10 flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full animate-pulse ${drawMode === 'blue' ? 'bg-blue-500' : 'bg-red-500'}`} />
                  <span className="text-[10px] font-black text-white uppercase tracking-widest whitespace-nowrap">
                    {drawMode === 'blue' ? 'Step 1: Draw Object Box' : 'Step 2: Draw Target Canvas'}
                  </span>
                </div>
              </div>
            )}
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-black text-white uppercase tracking-widest">Style Settings</span>
                    </div>
                    <button onClick={() => setShowSettings(false)} className="text-gray-300 hover:text-white transition-all bg-white/10 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-white/10 hover:border-white/30">Close</button>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3 mb-2">
                        <button 
                          ...
                          className={`py-3 rounded-xl text-[10px] font-black uppercase transition-all border-2 ${activeStylingTarget === 'blue' ? 'bg-blue-600/20 border-blue-500 text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.3)]' : 'bg-white/5 border-white/5 text-gray-500 hover:border-white/10'}`}
                        >
                          Source (Blue)
                        </button>
                        <button 
                          onClick={() => setActiveStylingTarget('red')}
                          className={`py-3 rounded-xl text-[10px] font-black uppercase transition-all border-2 ${activeStylingTarget === 'red' ? 'bg-red-600/20 border-red-500 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.3)]' : 'bg-white/5 border-white/5 text-gray-500 hover:border-white/10'}`}
                        >
                          Target (Red)
                        </button>
                      </div>
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                        填充模式 <span>{boxMode === 'fill' ? '填充' : '线框'}</span>
                      </label>
                      <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-xl">
                        {[
                          ...
                          <button
                            ...
                            className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${boxMode === mode.id ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:text-white'}`}
                          >
                            {mode.label}
                          </button>
                        ))}
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                        线条类型 <span>{(activeStylingTarget === 'blue' ? blueLineType : redLineType) === 'dashed' ? '虚线' : '实线'}</span>
                      </label>
                      <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-xl">
                        {[
                          { id: 'solid', label: '实线' },
                          ...
                          <button
                            key={type.id}
                            onClick={() => {
                              if (activeStylingTarget === 'blue') setBlueLineType(type.id as any);
                              else setRedLineType(type.id as any);
                            }}
                            className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${(activeStylingTarget === 'blue' ? blueLineType : redLineType) === type.id ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:text-white'}`}
                          >
                            {type.label}
                          </button>
                        ))}
                    ...
                    {(activeStylingTarget === 'blue' ? blueLineType : redLineType) === 'dashed' && (
                       <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                          虚线间距 <span>{activeStylingTarget === 'blue' ? `${blueDashLength}/${blueDashGap}` : `${redDashLength}/${redDashGap}`}</span>
                        </label>
                        <div className="flex flex-col gap-2">
                           <input 
                            type="range" min="1" max="20" step="1" 
                            title="虚线长度"
                            value={activeStylingTarget === 'blue' ? blueDashLength : redDashLength} 
                            onChange={(e) => {
                              if (activeStylingTarget === 'blue') setBlueDashLength(Number(e.target.value));
                           ...
                           <input 
                            type="range" min="1" max="20" step="1" 
                            title="间距大小"
                            value={activeStylingTarget === 'blue' ? blueDashGap : redDashGap}
                            onChange={(e) => {
                              if (activeStylingTarget === 'blue') setBlueDashGap(Number(e.target.value));
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                        线条粗细 <span>{activeStylingTarget === 'blue' ? blueLineWidth : redLineWidth}px</span>
                      </label>
                      <input 
                        type="range" min="1" max="25" step="1" 
                        value={activeStylingTarget === 'blue' ? blueLineWidth : redLineWidth}
                        onChange={(e) => {
                          if (activeStylingTarget === 'blue') setBlueLineWidth(Number(e.target.value));
                    ...
                    <div className="space-y-3">
                      ...
                      <input 
                        ...
                        value={boxOpacity} onChange={(e) => setBoxOpacity(Number(e.target.value))}
                        className="w-full accent-indigo-500 h-1 bg-white/10 rounded-full appearance-none cursor-pointer"
                      />
                    </div>

                    <div className="pt-4 border-t border-white/5 space-y-4">
                      <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">箭头细节调整</h4>
                      
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                          箭头颜色 <span style={{ color: arrowColor }}>{arrowColor}</span>
                        </label>
                        <div className="flex gap-2">
                          {['#fbbf24', '#3b82f6', '#ef4444', '#10b981', '#ffffff'].map(color => (
                            <button
                              key={color}
                      ...
                      <div className="space-y-3">
                        ...
                        <input 
                          ...
                          value={arrowWidth} onChange={(e) => setArrowWidth(Number(e.target.value))}
                      ...
                      <div className="space-y-3">
                        ...
                        <input 
                          type="range" min="0" max="100" step="5" 
                          value={arrowOpacity} onChange={(e) => setArrowOpacity(Number(e.target.value))}
                          className="w-full accent-yellow-500 h-1 bg-white/10 rounded-full appearance-none cursor-pointer"
                        />
                      </div>
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                          箭头样式 <span>{arrowDisplayMode === 'OUTLINE' ? 'Wireframe' : 'Straight Line'}</span>
                        </label>
                        <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-xl">
                          {[
                            ...
                            <button
                              key={mode.id}
                              onClick={() => setArrowDisplayMode(mode.id as any)}
                              className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${arrowDisplayMode === mode.id ? 'bg-yellow-600 text-white shadow-[0_0_10px_rgba(251,191,36,0.2)]' : 'text-gray-500 hover:text-white'}`}
                            >
                              {mode.label}
                            </button>
                          ))}
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                          箭头线型 <span>{arrowLineType === 'dashed' ? '虚线' : '实线'}</span>
                        </label>
                        <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-xl">
                          {[
                            { id: 'solid', label: '实线' },
                            ...
                            <button
                              ...
                              className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${arrowLineType === type.id ? 'bg-yellow-600 text-white' : 'text-gray-500 hover:text-white'}`}
                            >
                              {type.label}
                            </button>
                          ))}
                      ...
                      {arrowLineType === 'dashed' && (
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                            箭头虚线间距 <span>{arrowDashLength}/{arrowDashGap}</span>
                          </label>
                          <div className="flex flex-col gap-2">
                             <input 
                              type="range" min="1" max="20" step="1" 
                              title="虚线长度"
                              value={arrowDashLength} onChange={(e) => setArrowDashLength(Number(e.target.value))}
                              className="w-full accent-yellow-500 h-1 bg-white/10 rounded-full appearance-none cursor-pointer"
                             />
                             <input 
                              type="range" min="1" max="20" step="1" 
                              title="间距大小"
                              value={arrowDashGap} onChange={(e) => setArrowDashGap(Number(e.target.value))}
                              className="w-full accent-yellow-500 h-1 bg-white/10 rounded-full appearance-none cursor-pointer"
                             />
                          </div>
                        </div>
          ...
          <div className="shrink-0 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-[10px] font-black text-white uppercase tracking-widest" style={{ fontSize: 10 * zoomScale }}>标准化提示词输出</span>
              </div>
              <div className="flex items-center gap-4">
                 ...
                 >
                    {isEditing ? <Check size={12 * zoomScale} /> : <Move size={12 * zoomScale} />}
                    <span className="text-[9px] font-black uppercase tracking-tighter">{isEditing ? '完成退出' : '进入编辑'}</span>
                 </button>
                 {isEditing && (
                   <>
                     <button 
                     ...
                     >
                        ...
                        <span className="text-[9px] font-black uppercase tracking-tighter">重新选框</span>
                     </button>
                     <button 
                     ...
                     >
                        ...
                        <span className="text-[9px] font-black uppercase tracking-tighter">清空选框</span>
                     </button>
                   </>
                 )}
                 <div className="w-px h-3 bg-white/10" />
                 <button onClick={() => { setBlueBox({ x: 10, y: 10, width: 20, height: 20 }); setRedBox({ x: 40, y: 40, width: 20, height: 20 }); setBoxesVisible(true); setDrawMode(null); setIsEditing(false); }} className="text-gray-700 hover:text-white transition-colors">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                 <span className="text-[9px] font-black text-gray-600 uppercase tracking-widest italic">DYNAMIC_PAYLOAD_V2</span>
                 <span className="text-[8px] font-mono text-indigo-500 opacity-40">UTF-8:ENCODED</span>
              </div>
              <div 
              ...
              >
                {copied ? <Check size={16 * zoomScale} /> : <Copy size={16 * zoomScale} />}
              </button>
            </div>

            <div className="flex items-center gap-3 bg-white/[0.02] border border-white/5 rounded-2xl p-4" style={{ padding: 12 * zoomScale }}>
              <div className="p-2 bg-indigo-500/10 rounded-lg">
                <Info size={14 * zoomScale} className="text-indigo-400" />
              </div>
              <p className="text-[10px] text-gray-500 leading-relaxed font-sans" style={{ fontSize: 9 * zoomScale }}>
                蓝色框为<span className="text-blue-400 font-bold px-1">源物体选择框</span>，用于锁定主体。红色框为<span className="text-red-400 font-bold px-1">目标变换框</span>，定义最终位置与尺寸。
              </p>
            </div>
          </div>
        </div>
            ...
            >
               <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h2 className="text-2xl font-black text-white tracking-[0.2em] uppercase italic">DoubleBox_Precision_Workstation</h2>
                      <p className="text-xs text-gray-600 font-mono tracking-widest mt-1">ENGINE_STATUS: FULL_IMMERSION_MODE_ACTIVE</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <button 
               ...
               <div className="flex-1 flex gap-12 overflow-hidden">
                  <div 
                    ...
                    className={`flex-1 relative bg-black rounded-[40px] overflow-hidden border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.8)] ${drawMode ? 'cursor-crosshair' : ''}`}
                    onMouseDown={(e) => drawMode && onMouseDown(e)}
                  >
                    <div 
                    ...
                    {drawMode && (
                      <div className="absolute inset-0 bg-blue-500/5 pointer-events-none flex items-center justify-center">
                        <div className="px-8 py-4 bg-black/80 rounded-full border border-white/10 flex items-center gap-4">
                          <div className={`w-4 h-4 rounded-full animate-pulse ${drawMode === 'blue' ? 'bg-blue-500' : 'bg-red-500'}`} />
                          <span className="text-lg font-black text-white uppercase tracking-widest whitespace-nowrap">
                            {drawMode === 'blue' ? '第一步：框选源物体' : '第二步：框选目标位置'}
                          </span>
                        </div>
                      </div>
                    )}
                        ...
                        >
                          <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-4">
                              ...
                              <span className="text-sm font-black text-white uppercase tracking-[0.2em]">Style Settings</span>
                            </div>
                            <button 
                              onClick={() => setShowSettings(false)} 
                              className="text-gray-300 hover:text-white transition-all bg-white/10 px-4 py-1.5 rounded-xl text-xs font-bold uppercase tracking-widest border border-white/10 hover:border-white/30"
                            >
                              Close
                            </button>
                          </div>
                          ...
                          <div className="space-y-8">
                            <div className="space-y-4">
                              <div className="grid grid-cols-2 gap-4 mb-4">
                                <button 
                                  onClick={() => setActiveStylingTarget('blue')}
                                  className={`py-5 rounded-[24px] text-xs font-black uppercase transition-all border-2 shadow-2xl ${activeStylingTarget === 'blue' ? 'bg-blue-600/20 border-blue-500 text-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.2)] scale-105' : 'bg-white/5 border-white/5 text-gray-500 hover:border-white/10'}`}
                                >
                                  Source Box (Blue)
                                </button>
                                <button 
                                  onClick={() => setActiveStylingTarget('red')}
                                  className={`py-5 rounded-[24px] text-xs font-black uppercase transition-all border-2 shadow-2xl ${activeStylingTarget === 'red' ? 'bg-red-600/20 border-red-500 text-red-400 shadow-[0_0_30px_rgba(239,68,68,0.2)] scale-105' : 'bg-white/5 border-white/5 text-gray-500 hover:border-white/10'}`}
                                >
                                  Target Box (Red)
                                </button>
                              </div>
                            ...
                            <div className="space-y-4">
                              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                Box Mode <span>{boxMode === 'fill' ? 'Fill' : 'Outline'}</span>
                              </label>
                              <div className="flex gap-4 p-2 bg-white/5 border border-white/10 rounded-2xl">
                                {(['outline', 'fill'] as const).map(mode => (
                                  <button
                                    ...
                                    className={`flex-1 py-3 rounded-xl text-xs font-black uppercase transition-all ${boxMode === mode ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                                  >
                                    {mode}
                                  </button>
                                ))}
                            ...
                            <div className="space-y-4">
                              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                Stroke Type <span>{(activeStylingTarget === 'blue' ? blueLineType : redLineType) === 'dashed' ? 'Dashed' : 'Solid'}</span>
                              </label>
                              <div className="flex gap-4 p-2 bg-white/5 border border-white/10 rounded-2xl">
                                {(['solid', 'dashed'] as const).map(type => (
                                  <button
                                    key={type}
                                    onClick={() => {
                                      if (activeStylingTarget === 'blue') setBlueLineType(type);
                                      else setRedLineType(type);
                                    }}
                                    className={`flex-1 py-3 rounded-xl text-xs font-black uppercase transition-all ${(activeStylingTarget === 'blue' ? blueLineType : redLineType) === type ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                                  >
                                    {type}
                                  </button>
                                ))}
                            ...
                            {(activeStylingTarget === 'blue' ? blueLineType : redLineType) === 'dashed' && (
                               <div className="space-y-4">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                  Dash Length/Gap <span>{activeStylingTarget === 'blue' ? `${blueDashLength}/${blueDashGap}` : `${redDashLength}/${redDashGap}`}</span>
                                </label>
                                <div className="space-y-4">
                                  <input 
                            ...
                            <div className="space-y-4">
                              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                Stroke Width <span>{activeStylingTarget === 'blue' ? blueLineWidth : redLineWidth}px</span>
                              </label>
                              <input 
                                type="range" min="1" max="25" step="1" 
                                value={activeStylingTarget === 'blue' ? blueLineWidth : redLineWidth}
                            ...
                            <div className="pt-6 border-t border-white/10 space-y-6">
                              <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Scaling Arrow Style</label>
                              
                              <div className="space-y-4">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                  Arrow Color <span style={{ color: arrowColor }}>{arrowColor}</span>
                                </label>
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="space-y-4">
                                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                    Arrow Render Mode <span>{arrowDisplayMode === 'OUTLINE' ? 'Wireframe' : 'Straight Line'}</span>
                                  </label>
                                  <div className="flex gap-4 p-2 bg-white/5 border border-white/10 rounded-2xl">
                                    {(['LINE', 'OUTLINE'] as const).map(mode => (
                                      <button
                                        ...
                                        className={`flex-1 py-3 rounded-xl text-xs font-black uppercase transition-all ${arrowDisplayMode === mode ? 'bg-yellow-600 text-white shadow-[0_0_15px_rgba(251,191,36,0.3)] scale-105' : 'text-gray-500 hover:text-white'}`}
                                      >
                              ...
                              <div className="space-y-4">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                  Arrow Stroke Type <span>{arrowLineType === 'dashed' ? 'Dashed' : 'Solid'}</span>
                                </label>
                                <div className="flex gap-4 p-2 bg-white/5 border border-white/10 rounded-2xl">
                                  {(['solid', 'dashed'] as const).map(type => (
                                    <button
                                      ...
                                      className={`flex-1 py-3 rounded-xl text-xs font-black uppercase transition-all ${arrowLineType === type ? 'bg-yellow-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                                    >
                                      {type}
                                    </button>
                                  ))}
                              ...
                              {arrowLineType === 'dashed' && (
                                 <div className="space-y-4">
                                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex justify-between">
                                    Arrow Dash Length/Gap <span>{arrowDashLength}/{arrowDashGap}</span>
                                  </label>
                                  <div className="space-y-4">
                                    <input 
                                      type="range" min="1" max="50" step="1" 
                      ...
                      <div className="p-10 bg-white/[0.03] border border-white/10 rounded-[32px] flex flex-col gap-6 shadow-2xl shrink-0">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                             ...
                             <h4 className="text-sm font-black text-white uppercase tracking-widest">动态分析口令集</h4>
                          </div>
                          {/* Point Selection Helper */}
                          <div className="flex items-center gap-2 px-2 py-1 bg-black/40 rounded-full border border-white/10">
                            <button 
                              onClick={() => { setActiveStylingTarget('blue'); setShowSettings(true); }}
                            ...
                            />
                            <button 
                              onClick={() => { setActiveStylingTarget('red'); setShowSettings(true); }}
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">标注注释 (Node Notes)</label>
                            <span className="text-[10px] font-mono text-indigo-400/50 italic">Engineering Final</span>
                          </div>
                          <textarea 
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          >
                             ...
                             <span className="text-xs font-black uppercase tracking-widest">{isEditing ? '完成退出' : '进入编辑模式'}</span>
                          </button>
                          {isEditing && (
                        ...
                        <div className="bg-black/60 rounded-3xl p-8 border border-white/5 h-[300px] overflow-y-auto scrollbar-hide">
                           <pre className="text-xs font-mono text-emerald-400/90 whitespace-pre-wrap select-text">{prompt}</pre>
                        </div>
                        <button 
                          onClick={handleCopy}
                        ...
                        >
                           {copied ? <><Check size={20} /> 已复制</> : <><Copy size={20} /> 复制口令集</>}
                        </button>
                     </div>

                     <div className="p-8 bg-white/[0.01] border border-white/5 rounded-[32px]">
                        <h4 className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] mb-6">Execution Logic</h4>
                        <div className="space-y-6">
                            {[
                              { label: 'Rel_X', val: redBox.width > 0 ? Math.round(((blueBox.x - redBox.x) / redBox.width) * 1000) : 0 },
                              { label: 'Rel_Y', val: redBox.height > 0 ? Math.round(((blueBox.y - redBox.y) / redBox.height) * 1000) : 0 },
                              { label: 'Rel_W', val: redBox.width > 0 ? Math.round((blueBox.width / redBox.width) * 1000) : 0 },
                              { label: 'Rel_H', val: redBox.height > 0 ? Math.round((blueBox.height / redBox.height) * 1000) : 0 },
                            ].map((stat, i) => (
                             <div key={i} className="flex items-center justify-between">
                                <span className="text-[10px] font-mono text-gray-500">{stat.label}</span>
                                <div className="flex-1 mx-4 h-px bg-white/5" />
                                <span className="text-sm font-mono text-indigo-400 font-black">{stat.val}</span>
                             </div>
                           ))}
                        </div>
                     </div>
